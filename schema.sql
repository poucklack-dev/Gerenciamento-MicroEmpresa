-- Aurora Gestão - schema oficial do banco de dados
...
CREATE TABLE IF NOT EXISTS public.colaboradores (
    ...
    funcao TEXT,
    setor TEXT,
    chefe_direto_id BIGINT REFERENCES public.colaboradores(id) ON DELETE SET NULL,
    status TEXT,
    email TEXT,
    telefone TEXT,
    cnh TEXT,
...
CREATE TABLE IF NOT EXISTS public.banco_horas_ajustes (id BIGSERIAL PRIMARY KEY, periodo_id BIGINT NOT NULL REFERENCES public.banco_horas_periodos(id) ON DELETE CASCADE, minutos INTEGER NOT NULL, motivo TEXT, criado_em TIMESTAMP DEFAULT NOW());
CREATE TABLE IF NOT EXISTS public.banco_horas_colaborador (
 id BIGSERIAL PRIMARY KEY, periodo_id BIGINT NOT NULL REFERENCES public.banco_horas_periodos(id) ON DELETE CASCADE, colaborador_id BIGINT NOT NULL REFERENCES public.colaboradores(id) ON DELETE CASCADE,
 horas_previstas_minutos INTEGER, origem_horas TEXT NOT NULL DEFAULT 'registros' CHECK (origem_horas IN ('registros','manual')), horas_trabalhadas_manual_minutos INTEGER,
 origem_frequencia TEXT NOT NULL DEFAULT 'registros' CHECK (origem_frequencia IN ('registros','manual')), dias_presentes_manual INTEGER, dias_ausentes_manual INTEGER,
 excluido_periodo BOOLEAN NOT NULL DEFAULT FALSE, motivo_exclusao TEXT, excluido_por BIGINT REFERENCES public.usuarios(id) ON DELETE SET NULL, excluido_em TIMESTAMP,
 observacao TEXT, atualizado_por BIGINT REFERENCES public.usuarios(id) ON DELETE SET NULL, atualizado_em TIMESTAMP DEFAULT NOW(), UNIQUE(periodo_id,colaborador_id)
);
CREATE TABLE IF NOT EXISTS public.banco_horas_auditoria (id BIGSERIAL PRIMARY KEY, periodo_id BIGINT REFERENCES public.banco_horas_periodos(id) ON DELETE SET NULL, colaborador_id BIGINT REFERENCES public.colaboradores(id) ON DELETE SET NULL, usuario_id BIGINT REFERENCES public.usuarios(id) ON DELETE SET NULL, acao TEXT NOT NULL, valor_anterior TEXT, valor_novo TEXT, motivo TEXT, criado_em TIMESTAMP DEFAULT NOW());
