...
def main():
    ...
    try:
        ...
        cur.execute("""INSERT INTO colaboradores(nome,cargo,funcao,setor,status,email,telefone,data_admissao,cpf) SELECT v.* FROM (VALUES ('Beatriz Costa','Analista Financeiro','Financeiro','Financeiro','ativo','beatriz.costa@example.test','(11) 90000-1001',CURRENT_DATE-INTERVAL '18 months','111.111.111-11'),('Rafael Lima','Assistente Administrativo','Administrativo','Administrativo','ativo','rafael.lima@example.test','(11) 90000-1002',CURRENT_DATE-INTERVAL '9 months','222.222.222-22')) v(nome,cargo,funcao,setor,status,email,telefone,data_admissao,cpf) WHERE NOT EXISTS(SELECT 1 FROM colaboradores c WHERE c.cpf=v.cpf)""")
        cur.execute("UPDATE colaboradores SET setor=COALESCE(setor,funcao) WHERE cpf IN ('111.111.111-11','222.222.222-22')")
        cur.execute("UPDATE colaboradores SET chefe_direto_id=(SELECT id FROM colaboradores WHERE cpf='111.111.111-11') WHERE cpf='222.222.222-22'")
